...
public class JobDataRunner implements CommandLineRunner {
    ...
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
        ...
        if (askYes(reader, "사원 정보를 입력하시겠습니까? (y/N): ")) {
            while (keepGoing) {
                ...
                employeeService.addEmployee(name, salary, 
                    employeeService.getOrCreateDepartment(deptName, deptLoc).getDptId(), memo);
                anyInserted = true;
        ...
        // 검색 메뉴 개선
        System.out.println("\n=== 검색 옵션 ===");
        System.out.println("1. 모든 사원 출력");
        System.out.println("2. 키워드로 검색 (이름/위치/메모 포함)");
        System.out.println("3. 이름으로 정확히 검색");
        System.out.println("4. 검색 안 함");
        
        String choice = readRequired(reader, "선택하세요 (1-4): ");
        ...
        switch (choice) {
            case "1":
                ...
                break;
            case "2":
                String keyword = readRequired(reader, "검색할 키워드를 입력하세요: ");
                employeeService.printEmployeesByKeyword(keyword);
                break;
            case "3":
                String targetName = readRequired(reader, "검색할 사원 이름을 입력하세요: ");
            ...
            case "4":
                ...
                break;
            default:
                System.out.println("잘못된 선택입니다. 검색을 건너뜁니다.");
        }
        ...
        System.err.println(">>> [Runner] 콘솔 입력을 읽지 못했습니다.");
        return;
